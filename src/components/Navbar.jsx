...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white fixed bg-black px-4'>
        <div>
            <h1 className='font-signature text-5xl ml-2'>Gabe</h1>
        </div>

      <ul className='hidden md:flex'>
        {links.map(({id, link}) => (
        <li key={link.id} className='px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-100'
        >
            {link}
        </li>
        ))}
